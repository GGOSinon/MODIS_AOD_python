...
for f_name in sorted(glob(os.path.join(dir_name, '*.npy'))) :
    if not os.path.exists(f_name[:-4]+'_aea.png'):
        try :
            ...
            ax.drawparallels(np.arange(-90., 90., 10.), 
                             labels=[1, 0, 0, 1])
            ax.drawmeridians(np.arange(-180., 181., 15.), 
                             labels=[1, 0, 0, 1])
            ...
            plt.colorbar(cmap='bwr', fraction=0.0345, pad=0.02, 
                         ticks=(np.arange(0, vvmax+0.1, step=0.5)))
            x1,y1 = ax(171.5, 49.5)
            ...
            plt.savefig(f_name[:-4]+'_aea.png', bbox_inches='tight', dpi = 300)
